...
// With our implementation, we have the agent control a Lamp and set the Target's position within the Lamp's camera view
	...
	protected virtual void Start()
	{
		...
		logFilePath = Application.dataPath + @"\IO.txt";
	...
	public override void CollectObservations()
	{
        ...
        {
            file.WriteLine("Observations: " + obs.AsString());
        }
    }
	...
	// Moves the Lamp's servo positions from their initial positions to their final positions over a duration of 2 seconds
        ...
        using (System.IO.StreamWriter file = new System.IO.StreamWriter(logFilePath, true))
        {
            file.WriteLine(success ? "Success" : "Fail");
            file.WriteLine("Reward: " + reward);
            file.WriteLine("-----------\n");
        }
	...
	protected bool ResetAgentIfNotDone()
	{
		...
		print("DONE");
        using (System.IO.StreamWriter file = new System.IO.StreamWriter(@"C:\Users\Public\LampConsole\IO.txt", true))
        {
            file.WriteLine("DONE");
        }
        return false;
